import React from "react";
import "./Ring.css";

const Ring = () => {
  return (
    <body>
      <section>
        <div className="earth"></div>
        <div className="circle">
          <span style={{ "--i": 1 } as React.CSSProperties}>R</span>
          <span style={{ "--i": 2 } as React.CSSProperties}>T</span>
          <span style={{ "--i": 3 } as React.CSSProperties}>Q</span>
          <span style={{ "--i": 4 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 5 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 6 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 7 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 8 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 9 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 10 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 11 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 12 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 13 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 14 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 15 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 16 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 18 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 19 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 20 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 21 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 22 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 23 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 24 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 25 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 26 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 27 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 28 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 29 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 30 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 31 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 32 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 33 } as React.CSSProperties}>E</span>
          <span style={{ "--i": 34 } as React.CSSProperties}>E</span>
        </div>
      </section>
    </body>
  );
};

export default Ring;
